...
Eye 후보는 [`EYE_CANDIDATE_RESEARCH_PLAN.md`](EYE_CANDIDATE_RESEARCH_PLAN.md)의 D2-1 전수 발견 → D2-2 Hard Gate·사용자 후기 → D2-3 전수 Smoke → D2-4 점수화·1차 추천 순서를 따른다. D2 점수는 실제 정확도나 최종 모델 선택이 아니며, 상위 최대 3개만 D4 동일 조건 benchmark와 D5 모델·서버 결정으로 진입한다.
