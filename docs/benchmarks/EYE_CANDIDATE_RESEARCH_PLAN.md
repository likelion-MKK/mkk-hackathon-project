...
- 상태: Active — 사용자 승인에 따라 D2 실행 중
- 작성일: 2026-08-14
- D2-1 검색 시작 시각: 2026-08-14T23:03:20+09:00
- 소유자: 양유상
...
이 문서에서 `전수`는 인터넷에 존재하는 모든 구현을 절대적으로 증명한다는 뜻이 아니다. 위 검색 시작 시각, 아래 검색식, API page와 역추적 절차로 **공개 검색에서 재현 가능하게 발견한 프로젝트 적합 후보 전체**를 뜻한다.
...
- 각 분할은 최대 100건씩 마지막 page까지 읽고 조회 시각, 실제 query, 생성일 구간, `total_count`, `incomplete_results`, page, HTTP 상태, 오류와 재시도를 기록한다.
...
`search-manifest.json`에는 검색 시작 시각, 검색 채널·API version, query, 날짜 분할, page, 결과 수, 오류와 재시도 결과를 남긴다. `candidates.jsonl`의 각 후보에는 다음 필드가 있어야 한다.

- `candidate_id`, canonical URL, Hugging Face model ID, 발견 경로
- upstream·fork·wrapper 관계와 deduplication key
- RGB 웹캠 적합 여부와 model family·variant
...
| 입력 | 실행 중 생성한 synthetic no-face 입력과 라이선스 확인된 공식 positive fixture만 |

실행 전 실제 OS image, CPU/GPU, RAM/VRAM, driver/runtime과 시간대를 기록한다. GPU 필수 후보는 승인된 동일 L4 환경에서 실행하기 전까지 `deferred`이며 CPU 결과와 같은 표에서 성능 우열을 주장하지 않는다. 유료 cloud·GPU 자원은 별도 승인 없이 생성하지 않는다.

### 5.2 후보별 격리와 실행 순서

후보마다 독립 `pyproject.toml`·`uv.lock`·model cache를 사용하고 공용 Eye 서비스나 루트 lock 파일에 의존성을 섞지 않는다. source clone, weight, cache, 가상환경과 입력 이미지는 Git 밖의 후보별 임시 디렉터리에 둔다.

1. 고정 source·weight revision과 checksum을 확인한다.
2. 외부 code의 license, 설치 script, unsafe pickle과 remote-code 위험을 검토한다.
3. 격리 환경에서 package 설치와 model load를 실행한다.
4. `initialize → warmup → positive infer → no-face/invalid → dispose`를 실행한다.
5. 출력 shape·finite 값·no-face 의미와 calibration·viewport 변환 경로를 확인한다.
6. 최초 download 뒤 network를 차단하고 local asset만으로 다시 실행한다.
7. load·warmup 시간, 소량 반복 inference p50/p95, CPU/GPU와 RAM/VRAM peak를 관찰값으로 기록한다.
8. 명령, 종료 code, 오류 원문과 해결 또는 재평가 조건을 남긴다.

안전한 positive fixture를 확보하지 못하면 성공을 추정하지 않고 `deferred` 사유를 기록한다. Smoke는 설치·모델 로딩·출력·offline 경계만 확인한다. 후보마다 입력이나 반복 조건이 다르면 한 순위표의 속도 비교에 사용하지 않으며, 정답 좌표가 없는 결과를 정확도라고 부르지 않는다.
...
| 예상 서버 resource tier | CPU Small 5, CPU Medium 4, fractional GPU 3, full L4 2, 더 큰 구성 1 | 5 |

예상 서버 resource tier는 D2의 대리점수다. 실제 시간당·세션당 비용과 cloud·region은 D5에서 같은 날짜의 견적으로 다시 결정한다.
...
D4 진출 후보는 위 순위에서 최대 3개다. 적격 후보가 3개 미만이면 실패 후보로 수를 채우지 않는다. 추천 조건을 만족하는 후보가 없으면 `no qualifying recommendation`과 해소 조건을 기록한다.
...
D2-1~D2-4는 `feat/eye/d02-candidate-evaluation` 한 브랜치와 `dev` 대상 Draft PR 하나에서 수행한다. 승인 계획 동기화, D2-1, D2-2, 후보별 D2-3, D2-4를 각각 리뷰 가능한 커밋으로 구분한다. Draft PR은 진행 중 갱신하되 D2-4와 전체 검증 완료 후에만 최종 리뷰를 한 번 요청하며 merge하지 않는다.

완료 조건:

- [ ] 모든 검색식·조회 시각·날짜 분할·page·재시도와 중복 제거가 재현된다.
...
- [ ] Hard Gate 실패·deferred 후보가 점수표와 추천에 들어가지 않는다.
- [ ] 후기 증거가 부족한 후보의 후기 점수가 5/10 이하로 제한된다.
- [ ] 점수 합계, evidence coverage와 동점 규칙을 독립적으로 다시 계산할 수 있다.
- [ ] primary·다른 family fallback·필요 시 상용 가능 대안과 D4 상위 최대 3개가 명시된다.
- [ ] 원본 frame·image·base64·embedding·credential·token·weight가 Git·로그·artifact에 없다.
- [ ] Eye D2 validator, JSON/JSONL parsing, Markdown 상대 링크 검사, `python scripts/validate_contracts.py`와 `git diff --check`가 통과한다.
