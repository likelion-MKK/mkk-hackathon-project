...
- Hard Gate와 Smoke 통과 후보만 품질·실시간성·통합·법적/보안·후기·resource tier 100점 기준으로 평가한다.
...
D2-1~D2-4는 새로운 sprint 일자를 추가하는 이름이 아니라 D2 내부의 순차 Gate다. 이번 실행은 `feat/eye/d02-candidate-evaluation` 한 브랜치와 `dev` 대상 Draft PR 하나에서 진행하고, 각 Gate를 단계별 커밋으로 구분한다. 최종 리뷰 요청은 D2-4와 전체 검증이 끝난 뒤 한 번만 한다. 완료되지 않은 값을 추정해 다음 Gate로 넘기지 않는다.
