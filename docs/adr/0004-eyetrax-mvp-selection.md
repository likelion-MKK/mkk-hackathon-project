...
현재 브라우저 v2의 수집·세션 판정은 [로컬 보정 v2](../eye-calibration-local-v2.md)를 따른다.
2026-09-16 사용자가 판정 완화를 명시적으로 요청하여 `webcam-relaxed-v1`은 전체
유효 85%·오차 p50 15%·p95 30%, 개별 점 유효 70%·오차 p50 25%·p95 40%를 적용한다.
개별 점의 최소 15개 수집 표본과 invalid 분모 유지 조건은 보존한다. 아래 수치는 구형
고정 보정의 기준이며, 모델 선정 benchmark Gate나 역사적 실측 결과는 갱신하지 않는다.
...
LG 노트북 실제 얼굴 A/B에서 안정화 오차 p95가 원시 좌표보다 증가했다. 당시 jitter는
`rapid_shift_pending` 구간을 안정화 경로에서만 제외해 서로 다른 frame pair를 비교했으므로
개선 수치를 철회한다. 수정된 동일-pair 집계를 실제 카메라로 재실행하기 전까지 jitter 개선은
미검증이다. 따라서 이 브랜치에서는 안정화 구현과 명시적 실행 옵션만 유지하고 기본값을
`raw`로 둔다. 공식 3점 `tune()`과 자동 3경로 비교는 후속 작업으로 미룬다.
...
계산한다. jitter는 현재와 직전 frame에서 두 경로가 모두 유효한 경우에만 두 경로에 함께
추가한다. 어느 한 경로라도 invalid이면 공통 이전점을 비워 다음 값도 두 경로에서 동일하게
제외한다.
...
| jitter p95 | 재측정 필요 | 재측정 필요 | 기존 5.46% / 4.82%는 frame pair 불일치로 철회 |
...
Kiosk AOI Mapper가 없어 AOI hit는 `null`이다. 수정된 동일-pair jitter 집계는 실제 카메라로
아직 재실행하지 않았다. 확인된 오차 p95 증가와 미검증 jitter를 근거로 D06의 안정화 기능은
실험 옵션으로 마무리하고 기본 출력은 `raw`로 유지한다.
...
## 9. 로컬 보정 UX 후속 구현 — 2026-09-16

사용자는 시간 단축과 재미 요소 개선안의 로컬 구현을 승인했다.
[시선 보정 v2](../eye-calibration-local-v2.md)는 Dense5 화면 범위를 기본으로 유지하며
표적 동기화·가변 수집·독립 검증·일부 지점 보완을 적용한다. 16점은 비교 실험 옵션이다.
이 변경은 위의 역사적 실제 카메라 측정값을 갱신하지 않으며 모델 변경이나 배포 승인이 아니다.
새 방식의 완료 시간·다른 사용자 정확도·AOI hit는 실제 카메라 비교 후 별도로 판정한다.
