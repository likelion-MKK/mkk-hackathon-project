...
`official_product_page_verified_assets_pending` catalog 상태를 CHECK 제약에 추가한다.
`0005_supabase_backend_rls.sql`은 browser-facing `anon`/`authenticated` role의 public table
접근을 차단하고 backend role만 허용한다. observation/timeline 테이블은
...
- `GET /readyz`: DB, `0005` migration, canonical catalog 10개와 job intake readiness
