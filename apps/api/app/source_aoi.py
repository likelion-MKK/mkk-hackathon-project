...
ApprovalStatus = Literal["approved", "pending_review"]
...
class AoiApprovalV1(ContractModel):
    ...
    def approved_metadata_has_provenance(self) -> "AoiApprovalV1":
        ...
        if self.status == "pending_review" and (self.approved_by is not None or self.approved_at is not None):
            raise ValueError("pending-review AOI metadata cannot claim approval provenance")
        return self
...
def load_source_aoi_metadata(repository_root: Path, video_id: str) -> SourceAoiMetadataV1 | None:
    path = repository_root / "data" / "lookbooks" / video_id / "source-aoi-metadata-v1.json"
    if not path.is_file():
        return None
    return SourceAoiMetadataV1.model_validate(json.loads(path.read_text(encoding="utf-8")))


def load_product_matching_catalog(
    ...
    path = path or (
        ...
        / "data"
        / "products"
        / "mcm-recommendation-matching-profiles-v1.json"
    )
