...
import { canContinueAfterCalibration } from "./calibration-plan.ts";

test("weak and missing signals continue without treating cancellation or protocol faults as success", () => {
  for (const reason of ["validation_accuracy_failed", "validation_signal_insufficient", "validation_samples_insufficient",
    ...
    const result = { calibration_id: "cal-1", valid: false, reason };
    assert.equal(canContinueAfterCalibration(result), true);
    assert.equal(result.valid, false);
  }
  for (const reason of ["calibration_cancelled", "viewport_changed", "invalid_calibration_request", "eye_worker_invalid_response"]) {
    assert.equal(canContinueAfterCalibration({ calibration_id: "cal-1", valid: false, reason }), false);
  }
});
