import type { CalibrationPattern, CalibrationResult, NormalizedPoint } from "./kiosk-types.ts";
...
// Quality and detection failures no longer reject the visitor. A missing model
// will emit unavailable observations; cancellation/protocol faults stay errors.
const CONTINUABLE_CALIBRATION_REASONS = new Set([
  "face_position_not_ready", "face_position_unavailable", "unstable_head", "no_face",
  "insufficient_calibration_samples", "validation_samples_insufficient",
  "validation_signal_insufficient", "validation_accuracy_failed",
  "validation_signal_and_accuracy_failed", "quality_gate_failed", "calibration_timed_out",
  "eye_not_connected", "eye_worker_unavailable",
]);

export function canContinueAfterCalibration(result: CalibrationResult): boolean {
  return result.valid || (result.reason !== null && CONTINUABLE_CALIBRATION_REASONS.has(result.reason));
}
