...
$outputPath = Join-Path $repositoryRoot "artifacts\recommendation\openai-luna-max\diagnostic-full-v5-optimized-no-timeout.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Luna v5 optimized diagnostic failed with exit code $LASTEXITCODE."
    }
