...
def test_synchronized_weak_model_keeps_quality_failure_and_bounds_inference(tmp_path, monkeypatch, failure):
    ...
    try:
        ...
        sample = adapter.infer(make_frame(100, 50), FrameContext())
        if failure == "accuracy":
            assert result.reason == "validation_accuracy_failed"
            assert adapter.inference_ready and sample.valid
            assert sample.confidence == .35
            assert (sample.screen_x_norm, sample.screen_y_norm) == (.5, .5)
            assert sample.model_revision.endswith("+low-confidence")
            assert adapter.calibration_summary["failed_checks"]
            assert adapter.calibration_summary["inference_ready"] is True
        else:
            assert not adapter.inference_ready and not sample.valid
            assert sample.screen_x_norm is None
