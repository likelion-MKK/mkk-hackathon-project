...
def test_worker_discards_a_fitted_model_when_its_quality_gate_fails(monkeypatch):
    ...
    assert not app.calibrated and not app.inference_ready
    assert app.calibration_id is None and app._calibrated_binding is None
    assert app._adapter.discarded
