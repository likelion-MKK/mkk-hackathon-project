...
class EyeTraxAdapter:
    ...
    def calibrate_synchronized(self, request: CalibrationRequest, source, profile_id: str) -> CalibrationResult:
        ...
        engine = SessionCalibration(estimator, source, fit, profile_id=profile_id,
                                    repair_accuracy=profile_id == "fixed-dense5-v2")
        try:
            engine.run()
            self._calibrated = True
            ...
            if engine.model_fitted and str(exc) in {
                "validation_samples_insufficient", "validation_signal_insufficient",
                "validation_accuracy_failed", "validation_signal_and_accuracy_failed",
                "quality_gate_failed", "calibration_timed_out", "insufficient_calibration_samples",
            }:
                self._calibrated = True
                # A conservative policy weight, NOT a measured accuracy percentage.
                self._calibration_confidence = 0.35
    ...
    def inference_ready(self) -> bool:
        """A fitted session model may be usable even if its quality check failed."""
